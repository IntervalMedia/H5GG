...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Building GlobalView for all THEOS_PACKAGE_SCHEME types${NC}"
echo -e "${BLUE}========================================${NC}"

# Create build output directory
BUILD_DIR="$PWD/build_output_globalview"
rm -rf "$BUILD_DIR"
mkdir -p "$BUILD_DIR"
...
if [ $? -eq 0 ]; then
    ...
    if [ -n "$LATEST_DEB" ]; then
        cp "$LATEST_DEB" "$BUILD_DIR/libH5GGApp-rootfull.deb"
        echo -e "${GREEN}✓ Rootfull build completed: $BUILD_DIR/libH5GGApp-rootfull.deb${NC}"
    fi
else
...
if [ $? -eq 0 ]; then
    LATEST_DEB=$(ls -tr1 packages/*.deb 2>/dev/null | tail -1)
    if [ -n "$LATEST_DEB" ]; then
        cp "$LATEST_DEB" "$BUILD_DIR/libH5GGApp-rootless.deb"
        echo -e "${GREEN}✓ Rootless build completed: $BUILD_DIR/libH5GGApp-rootless.deb${NC}"
    fi
else
...
if [ $? -eq 0 ]; then
    LATEST_DEB=$(ls -tr1 packages/*.deb 2>/dev/null | tail -1)
    if [ -n "$LATEST_DEB" ]; then
        cp "$LATEST_DEB" "$BUILD_DIR/libH5GGApp-roothide.deb"
        echo -e "${GREEN}✓ Roothide build completed: $BUILD_DIR/libH5GGApp-roothide.deb${NC}"
    fi
else
