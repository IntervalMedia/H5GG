...
@end
#else
typedef struct FloatMenu FloatMenu;
#endif

// Declared here, defined once in Tweak.mm
extern FloatMenu* floatH5;

static inline BOOL RBIsUserAppExecutablePath(const char *rawPath) {
    ...
    if ([path hasPrefix:kRootlessPrefix]) {
        path = [path substringFromIndex:kRootlessPrefix.length];
    }
    BOOL hasVarPrefix = [path hasPrefix:@"/private/var/"] || [path hasPrefix:@"/var/"];
    return hasVarPrefix && [path containsString:@"/Application/"];
}

static inline int JJSearchTypeLength(int jjtype) {
    switch(jjtype) {
...
-(void)setFloatTolerance:(NSString*)value
{
    ...
    [(id<H5Alerting>)floatH5 alert:Localized(@"浮点误差格式错误")];
        return;
    }
...
-(NSArray*)getResults:(int)maxCount param1:(int)skipCount {
    ...
    try {
        ...
        [(id<H5Alerting>)floatH5 alert:Localized(@"错误:内存不足!")];
    }
    ...
    for(map<void*,int8_t>::iterator it = results.begin(); it != results.end(); ++it) {
        ...
        // Use a larger buffer for string types during display
        if (jjtype == JJ_Search_Type_CString || jjtype == JJ_Search_Type_UTF16) {
            // Read up to 64 bytes for display purposes
            UInt8 valuebuf[64] = {0};
            vm_size_t size = 0;
            vm_read_overwrite(self.engine->task, (uint64_t)address, sizeof(valuebuf)-2, (vm_address_t)valuebuf, &size);
...
-(int)ggtype2jjtype:(NSString*)type
{
    ...
    // New String Types
    else if([type isEqualToString:@"utf8"])
        return JJ_Search_Type_CString;
    else if([type isEqualToString:@"utf16"])
        return JJ_Search_Type_UTF16;
...
-(NSString*)formartValue:(void*)value byType:(NSString*)type
{
    ...
    // String Formatting
        ...
        // Assume basic sanity check for length, here we just read from the buffer passed in
        NSString* str = [[NSString alloc] initWithBytes:value length:wcslen((wchar_t*)value)*2 encoding:NSUTF16LittleEndianStringEncoding];
        // Fallback if wcslen fails or buffer isn't null terminated cleanly in read window
        if (!str) str = [[NSString alloc] initWithData:[NSData dataWithBytes:value length:32] encoding:NSUTF16LittleEndianStringEncoding];
        return str ? str : @"";
    }
    else {
        [(id<H5Alerting>)floatH5 alert:Localized(@"不支持的数值类型")];
        return nil;
    }
}
...
-(int)parseValue:(void*)valuebuf from:(NSString*)value byType:(NSString*)type
{
    ...
    // New String Parsing
        ...
        if (len > 255) len = 255; // Safety cap
        memcpy(valuebuf, cStr, len);
        ((char*)valuebuf)[len] = '\0';
        JJType = JJ_Search_Type_CString;
    }
    else if([type isEqualToString:@"utf16"]) {
        NSData* data = [value dataUsingEncoding:NSUTF16LittleEndianStringEncoding];
        size_t len = data.length;
        if (len > 254) len = 254; // Safety cap
        memcpy(valuebuf, data.bytes, len);
        ((char*)valuebuf)[len] = 0;
        ((char*)valuebuf)[len+1] = 0; // Double null terminator for UTF16
        JJType = JJ_Search_Type_UTF16;
    }
    else {
        [(id<H5Alerting>)floatH5 alert:Localized(@"不支持的数值类型")];
        return 0;
    ...
    if(pvaluerr && pvaluerr[0]) {
        [(id<H5Alerting>)floatH5 alert:Localized(@"数值格式错误或与类型不匹配")];
        return 0;
    }
...
{
    // Skip regex range parsing for strings, as ranges usually apply to numbers
    if ([type isEqualToString:@"utf8"] || [type isEqualToString:@"utf16"]) {
        return [self parseValue:valuebuf from:value byType:type];
    }
    ...
    if([result numberOfRanges] != 3) {
        ...
        if(!jjtype) return 0;
        int len = JJSearchTypeLength(jjtype);
...
-(void)searchNumber:(NSString*)value param2:(NSString*)type param3:(NSString*)memoryFrom param4:(NSString*)memoryTo
{
    ...
    [(id<H5Alerting>)floatH5 alert:Localized(@"数值搜索:参数有误")];
        return;
    }
    
    // Increased buffer to accommodate strings
    UInt8 valuebuf[256];
    ...
    [(id<H5Alerting>)floatH5 alert:Localized(@"搜索范围需以0x开头十六进制数")];
        return;
    }
    ...
    [(id<H5Alerting>)floatH5 alert:Localized(@"内存搜索范围格式错误")];
        return;
    }
    
    if(self.firstSearchDone && self.engine->getResultsCount()==0) {
    [(id<H5Alerting>)floatH5 alert:Localized(@"改善搜索失败: 当前列表为空, 请清除后再重新开始搜索")];
        return;
    }
...
-(void)searchNearby:(NSString*)value param2:(NSString*)type param3:(NSString*)range
{
    ...
    [(id<H5Alerting>)floatH5 alert:Localized(@"邻近搜索:参数有误")];
        return;
    }
    
    if(![range hasPrefix:@"0x"]) {
    [(id<H5Alerting>)floatH5 alert:Localized(@"邻近范围需以0x开头十六进制数")];
        return;
    }
    ...
    [(id<H5Alerting>)floatH5 alert:Localized(@"邻近范围格式错误")];
        return;
    }
    
    if(searchRange<2 || searchRange>4096) {
    [(id<H5Alerting>)floatH5 alert:Localized(@"邻近范围只能在2~4096之间")];
        return;
    }
    
    if(self.engine->getResultsCount()==0) {
    [(id<H5Alerting>)floatH5 alert:Localized(@"邻近搜索错误: 当前列表为空, 请清除后再重新开始搜索")];
        return;
    }
...
-(NSString*)getValue:(NSString*)address param2:(NSString*)type
{
    ...
    [(id<H5Alerting>)floatH5 alert:Localized(@"读取失败:地址格式有误!")];
        return @"";
    }
    
    // Buffer for reading
    UInt8 valuebuf[256] = {0};
    
    // For strings, we read more bytes
    if (jjtype == JJ_Search_Type_CString || jjtype == JJ_Search_Type_UTF16) {
        vm_size_t size = 0;
        kern_return_t kr = vm_read_overwrite(self.engine->task, addr, sizeof(valuebuf)-2, (vm_address_t)valuebuf, &size);
...
-(BOOL)setValue:(NSString*)address param2:(NSString*)value param3:(NSString*)type
{
    ...
    if((pvaluerr && pvaluerr[0]) || !addr) {
    [(id<H5Alerting>)floatH5 alert:Localized(@"修改失败:地址格式有误!")];
        return FALSE;
    }
...
-(int)editAll:(NSString*)value param3:(NSString*)type
{
    ...
    if(self.engine->getResultsCount()==0) {
    [(id<H5Alerting>)floatH5 alert:Localized(@"修改全部: 结果列表为空!")];
        return 0;
    }
